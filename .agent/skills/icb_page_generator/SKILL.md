...
41. **Layout-first 對稿模式：** /create 與 /create-home 都可切換 Layout-first（只做結構骨架），圖片位置用灰色色塊占位、不放彩圖；若使用者明確要求 first-layout / layout-first / 先看版型 / 先看骨架，必須先輸出灰階占位對稿版，內容至少包含 section 順序、左右欄配置、每區目的，並附可視化佈局圖（ASCII / 結構圖皆可）；未確認前禁止直接進正式 XML/SCSS。
