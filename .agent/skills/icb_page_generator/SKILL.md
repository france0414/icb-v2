...
26. **預覽資產規則：** 凡 `/create`、`/create-home`、`/page`、`/page-home` 需要做預覽時，不可寫死任何 Odoo asset CSS 路徑。正確流程是：若已知案件網址，應先從該網址當前頁面重新抓取實際引用中的 CSS bundle（如 web.assets_common / web.assets_frontend 等）並更新預覽設定，再載入本次輸出的 XML/SCSS；因為客戶只要修改 SCSS、theme 或 assets，bundle 路徑就可能改變。若沒有案件網址，才允許使用 fallback CSS 作為近似預覽。
